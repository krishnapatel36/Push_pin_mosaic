...
import os
import base64
st.set_page_config(page_title='Push Pin Art', page_icon=":rocket:")
...
def main():
    ...
    input_image = st.sidebar.file_uploader("Upload Image", type=["png", "jpg", "jpeg", "gif"])
    
    num_colors = st.sidebar.number_input("Number of Colors", min_value=1, value=1, step=1)

    color_mapping = []
    for i in range(num_colors):
        color = st.sidebar.color_picker(f"Select Color {i+1}")
        color_mapping.append(hex_to_rgb(color))
    
    if st.sidebar.button("Apply Dithering"):
        if not input_image:
